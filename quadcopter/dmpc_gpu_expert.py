...
class BatchedGPUDMPC:
    ...
    def __init__(
        ...
        # Cached Cholesky factor for fixed-KKT scenarios (set by precompute_cholesky()).
        self._L_fixed: torch.Tensor | None = None
        self._L_fixed_T: torch.Tensor | None = None
    ...
    @torch.no_grad()
    def precompute_cholesky(self) -> None:
        """Cache the Cholesky factor for the fixed-KKT case.

        Valid when the A matrix does not vary across batch entries:
        - ``max_collision_slots == 0``: A == _A_fixed always.
        - ``max_collision_slots > 0`` with zero A_col (free-mode agents):
          collision rows get ``[0 | -I]``, which is also fixed.
        Calling this makes ``solve()`` skip the per-call Cholesky build.
        """
        A = self._A_fixed.clone()  # (m, n_var)
        if self.max_collision_slots > 0:
            coll0 = self.n_fixed
            coll1 = coll0 + self.max_collision_slots
            eye_s = torch.eye(self.max_collision_slots, device=self.device, dtype=torch.float32)
            A[coll0:coll1, self.n_bez:] = -eye_s
        At = A.T.contiguous()
        KKT = self._H + self.rho * (At @ A)
        eye_v = torch.eye(self.n_var, device=self.device, dtype=torch.float32)
        KKT = 0.5 * (KKT + KKT.T) + self.reg * eye_v
        try:
            L = torch.linalg.cholesky(KKT)
        except RuntimeError:
            L = torch.linalg.cholesky(KKT + 10.0 * self.reg * eye_v)
        self._L_fixed = L
        self._L_fixed_T = L.T.contiguous()
    ...
    def solve(
        ...
        if self._L_fixed is not None:
            L = self._L_fixed.unsqueeze(0).expand(B, -1, -1)
            L_T = self._L_fixed_T.unsqueeze(0).expand(B, -1, -1)
        else:
...
class GPUDMPCExpert(DMPCExpert):
    def __init__(self, num_drones: int, num_envs: int, params: DMPCParams | None = None, device="cpu",
        ...
        self._enable_gpu_admm(
            max_envs=max(self.p.max_envs, self.E),
            rho=self.p.rho,
    ...
    def _enable_gpu_admm(
        ...
        # Penalty CA fallback solvers (admm_collision_slots=0, fewer iters).
        # Used when the main soft-slot solve fails to converge.
        def _make_penalty_solver(H_bez: np.ndarray) -> BatchedGPUDMPC:
            return BatchedGPUDMPC(
                n_bez=self.n_bez,
                H_bez=H_bez,
                A_cont=self.A_continuity,
                ...
                max_B=max_envs * self.N,
                rho=rho,
                admm_iters=self.p.admm_penalty_iters,
                alpha=alpha,
                device=device,
                eps_abs=self.p.admm_eps_abs,
                eps_rel=self.p.admm_eps_rel,
                reg=self.p.reg,
                max_collision_slots=0,
                quad_coll=self.p.admm_quad_coll,
                lin_coll=self.p.admm_lin_coll,
            )
        self._gpu_solvers_penalty: dict[str, BatchedGPUDMPC] = {
            "free":      _make_penalty_solver(self._H_bez_free),
            "obs":       _make_penalty_solver(self._H_bez_obs),
            "emergency": _make_penalty_solver(self._H_bez_emergency),
        }
        # Precompute Cholesky for fixed-KKT solvers:
        # - penalty solvers: max_collision_slots=0 → A = _A_fixed always
        # - free-mode main solver: free agents always have A_col=0 → same fixed A
        for solver in self._gpu_solvers_penalty.values():
            solver.precompute_cholesky()
        self._gpu_solvers["free"].precompute_cholesky()
        ...
        self._st_U          = np.zeros((max_envs, N_, self.n_bez), dtype=np.float32)
        self._st_u_pred     = np.zeros((max_envs, N_, K, 3),       dtype=np.float32)
        self._st_seeds      = np.zeros((max_envs, N_, deg1, 3),    dtype=np.float32)
        ...
        # Pre-allocated scratch buffers — reused every compute() call to avoid
        # per-call heap allocation.  Sized to max_envs (not current E) so they
        # never need to be resized.
        N_nbr = max(N_ - 1, 0)
        self._buf_nbr_preds   = np.empty((max_envs, N_, N_nbr, K, 3), dtype=np.float32)
        self._buf_valid_nbr   = np.zeros((max_envs, N_, N_nbr), dtype=bool)
        self._buf_prio_scores = np.zeros((max_envs, N_), dtype=np.float64)
        self._buf_nbr_prio    = np.zeros((max_envs, N_, N_nbr), dtype=np.float64)
    ...
    def _plan_batched_gpu(
        ...
        qc     = self.p.admm_quad_coll           # soft-slot slack cost weight
        qc_pen = self.p.admm_penalty_quad_coll   # penalty CA gradient weight (matches original)
        B      = len(e_arr)
        ...
        amin_v = np.asarray(self.p.amin, dtype=np.float32)
        amax_v = np.asarray(self.p.amax, dtype=np.float32)
        h_dyn  = self.p.h
        ...
        n_seeds    = n_eq - n_eq_c
        seeds_flat = seeds_batch.reshape(B, n_seeds)
        ...
        x0_t    = torch.from_numpy(x0_batch).to(dev)     # (B, 6)
        goal_t  = torch.from_numpy(goal_batch).to(dev)   # (B, 3)
        own_t   = torch.from_numpy(own_batch).to(dev)    # (B, K, 3)
        nbr_t   = torch.from_numpy(nbr_batch).to(dev)    # (B, N-1, K, 3)
        ...
        if has_coll.any():
            ...
            # Always compute penalty gradient — used directly (penalty path) or as fallback.
            pg = -2.0 * qc_pen * torch.einsum('bn,bnm->bm', viol, dg_dU)  # (B_c, n_bez)
            q_penalty_t[coll_idx] += pg
            if use_soft_slots:
                coll_idx_np = coll_idx.cpu().numpy()
                dg_np = dg_dU.detach().cpu().numpy()
        ...
        # Soft-slot path: penalty gradient is kept separate for fallback use, not added to main q.
        if not use_soft_slots:
            q_t += q_penalty_t
        ...
        # Penalty CA fallback: re-solve failed agents with penalty CA (collision_slots=0).
        if use_soft_slots and self.p.enable_penalty_fallback and fallback_mask.any():
            failed_rows = np.nonzero(fallback_mask)[0]
            q_fallback_t = q_t.clone()
            q_fallback_t[failed_rows] += q_penalty_t[failed_rows]
            for mode_code, solver_name in ((0, "free"), (1, "obs"), (2, "emergency")):
                fb_mask = mode_np[failed_rows] == mode_code
                fb_rows = failed_rows[fb_mask]
                if len(fb_rows) == 0:
                    continue
                rows_t = torch.from_numpy(fb_rows.astype(np.int64)).to(dev)
                pen_solver = self._gpu_solvers_penalty[solver_name]
                U_fb_t, st_fb_t = pen_solver.solve(
                    q_fallback_t[rows_t], beq_t[fb_rows], idx_t[fb_rows]
                )
                U_raw[fb_rows] = U_fb_t.cpu().numpy()
                solve_status[fb_rows] = st_fb_t.cpu().numpy().astype(np.int32)
            fallback_type[failed_rows] = "penalty"
            # Recompute fallback_mask — any remaining failures fall through to brake_hover.
            fallback_mask = (solve_status < 0) | (~np.isfinite(U_raw).all(axis=1))
        ...
        if _do_log:
            ...
            n_fb_penalty = int((fallback_type == "penalty").sum())
            n_fb_other = int(fallback_mask.sum()) - n_fb_shift - n_fb_brake - n_fb_penalty
            ...
            print(
                ...
                f"  fallback=penalty:{n_fb_penalty}/shift:{n_fb_shift}/brake:{n_fb_brake}/other:{n_fb_other}"
    ...
    def compute(
        ...
        Args:
            ...
            sync_state_dict: if True, rebuild the base-class ``_state`` dict
                after planning (needed by test scripts that read ``_state``
                for acceleration computation).  Defaults to False for speed.
        ...
        # ── Input → numpy (float32, not float64) ──────────────────────────
        pos_np  = pos_w.detach().cpu().numpy().astype(np.float32)
        vel_np  = vel_w.detach().cpu().numpy().astype(np.float32)
        goal_np = goal_w.detach().cpu().numpy().astype(np.float32)
        if env_origins is not None:
            origins_np = env_origins.detach().cpu().numpy().astype(np.float32)
        ...
        # env_arr: avoid building a Python list when processing all envs.
        if env_ids is None:
            env_arr  = np.arange(E, dtype=np.intp)
            env_list = None          # sentinel: means "all envs"
        else:
            env_arr  = env_ids.detach().cpu().numpy().astype(np.intp)
            env_list = env_arr.tolist()
        ...
        # Phase 1a: reset flags (numpy fancy-index, no Python loop)
        ...
        # Reuse pre-allocated buffers — write only the active slice.
        self._buf_prio_scores[env_arr] = priority_scores_local
        self._st_priority_scores[env_arr] = priority_scores_local

        # Phase 1b: neighbour predictions via fancy indexing (no Python loop)
        _tb = time.perf_counter()
        upred_all = self._st_u_pred[env_arr]    # (E_n, N, K, 3)
        has_all   = self._st_has[env_arr]        # (E_n, N)
        pos_tiled = np.broadcast_to(
            pos_np_local[env_arr, :, np.newaxis, :],
            (E_n, N, K, 3),
        ).copy()  # copy needed because np.where requires contiguous
        upred_safe = np.where(
        ...
        nbr_priority_local = priority_scores_local[:, self._neighbor_idx] # (E_n, N, N-1)

        # Write into pre-allocated buffers (no new heap allocation).
        self._buf_nbr_preds[env_arr]  = nbr_preds_local
        self._buf_valid_nbr[env_arr]  = valid_nbr_local
        self._buf_nbr_prio[env_arr]   = nbr_priority_local
        _t_nbr = time.perf_counter() - _tb

        # Phase 2: replan condition (numpy, no dict loop)
        ...
        if replan_e_arr.size > 0:
            self._plan_batched_gpu(
                ...
                self._buf_nbr_preds, self._buf_valid_nbr,
                self._buf_prio_scores, self._buf_nbr_prio,
        ...
        all_U   = self._st_U[e_all, i_all]   # (E_n*N, n_bez) — already float32
        all_sub = (self._st_steps[e_all, i_all] % self.n_substeps).astype(np.int32)
        self._st_steps[e_all, i_all] += 1

        if origins_np is not None:
            all_ori = np.repeat(origins_np[env_arr], N, axis=0)  # already float32
        else:
        ...
        # Sync _state dict only when explicitly requested (e.g. test scripts).
        if sync_state_dict:
            _env_list_for_sync = list(range(E)) if env_list is None else env_list
            self._sync_state_dict_from_arrays(_env_list_for_sync)

        # Write results — bulk CPU→GPU transfers.
        ...
        if env_ids is None:
            ...
            env_t = torch.from_numpy(env_arr.astype(np.int64)).to(device)
