...
from policyflow_torch.modules.normalizer import EmpiricalNormalization
from policyflow_torch.modules.utils import get_activation
from policyflow_torch.modules.transformer import Transformer
from policyflow_torch.utils.benchmarkable import Benchmarkable
from policyflow_torch.utils.utils import squeeze_preserve_batch
...
class Network(Benchmarkable, nn.Module):
    ...
    recurrent_modules = {
        recurrent_module_lstm: nn.LSTM,
        recurrent_module_transformer: Transformer,
    }
    ...
    def forward(self, x: torch.Tensor, hidden_state=None) -> torch.Tensor:
        ...
        """
        assert (
            hidden_state is None or self._recurrent
        ), "Cannot pass hidden state to non-recurrent network."

        input = self._normalization(x.to(self.device))

        if self._recurrent:
            current_hidden_state = (
                self.hidden_state if hidden_state is None else hidden_state
            )
            current_hidden_state = (
                current_hidden_state[0].to(self.device),
                current_hidden_state[1].to(self.device),
            )
    ...
    def reset_hidden_state(self, indices: torch.Tensor) -> None:
        ...
        self.hidden_state[0][:, indices] = torch.zeros(
            len(indices), self._features[0].hidden_size, device=self.device
        )
        self.hidden_state[1][:, indices] = torch.zeros(
            len(indices), self._features[0].hidden_size, device=self.device
        )
    ...
    def reset_full_hidden_state(self, batch_size=None) -> None:
        ...
        if batch_size is None:
            ...
            layer_count, hidden_size = (
                self._features[0].num_layers,
                self._features[0].hidden_size,
            )
            self.hidden_state = (
    ...
    def _init(
        self, layers: List[nn.Module], fade: bool = True, gain: float = 1.0
    ) -> List[nn.Module]:
        ...
        last_layer_idx = (
            ...
            - 1
            - next(
                i for i, l in enumerate(reversed(layers)) if isinstance(l, nn.Linear)
            )
        )
