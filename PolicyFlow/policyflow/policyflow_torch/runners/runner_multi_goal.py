...
class MultiGoalRunner:
    ...
    def evaluate(self, steps: int, return_epochs: int = 100) -> float:
        ...
        ## DEBUG
        self._env.pos_state_buf[:] = torch.tensor([[0.0, 0.0]], device=self._device).expand(
            self._env.num_envs, 2
        )
        self._env.obs_buf[:] = torch.cat((self._env.pos_state_buf, self._env.vel_state_buf), dim=-1)
        obs_dict["actor_observations"] = self._env.obs_buf.clone()
        #########################################################
        ...
        with torch.inference_mode():
            for step in tqdm.tqdm(range(steps), disable=False, file=sys.stdout, desc="step"):
                actions = policy(obs_dict)
                # step the environments
                next_obs_dict, rewards, dones, env_info = self._env.step(actions)
                
                obs_dict_list.append(copy.deepcopy(obs_dict))
                actions_list.append(copy.deepcopy(actions))
                resets_list.append(copy.deepcopy(dones))
                ...
                # Calculate and print mean reward similar to train() function
            mean_reward = (
                sum(episode_statistics["returns"]) / len(episode_statistics["returns"])
                if len(episode_statistics["returns"]) > 0
                else 0.0
            )
            print(f"Mean reward: {mean_reward:.2f}")
        return obs_dict_list, actions_list, resets_list
    ...
    def train(
        ...
        for _iter in tqdm.tqdm(
            range(max_iterations), disable=False, file=sys.stdout, desc="iter", leave=False
        ):
            for _ in tqdm.tqdm(
                range(rollouts), disable=False, file=sys.stdout, desc="step", leave=False
            ):
                with torch.inference_mode():
                    # compute actions
